...
{
    /// <summary>
    /// Реализация фрактала с раширинными возможностями.
    /// Фрактал принимет на фход правила и интерпитации с командами для литералов. При наследовании можно реализовать выполнение кастомных команды.
    ...
    /// </summary>
    public class FractalExt : IFractal
    {
        ...
        public FractalExt(string axiom, IEnumerable<string> rules, IEnumerable<string> interpretations)
        {
            Axiom = axiom;

            Rules = new Dictionary<char, FractalRule>();
            foreach (string rule in rules)
            {
                FractalRule r = new FractalRule(rule);
                Rules.Add(r.Literal, r);
            }

            Interpretations = new Dictionary<char, FractalInterpretation>();
            foreach (string interpretation in interpretations)
            {
                FractalInterpretation i = new FractalInterpretation(interpretation);
        ...
        /// <param name="interpretations">Интерпритации литералов</param>
        public FractalExt(string axiom, string rule, IEnumerable<string> interpretations) :
        ...
        /// </summary>
        public Dictionary<char, FractalRule> Rules { get; }

        /// <summary>
        /// Интерпритации, соответствующие каждому литералу.
        /// </summary>
        public Dictionary<char, FractalInterpretation> Interpretations { get; }
        ...
        /// <summary>
        /// Стек с сохраненными состояниями при отрисовки фрактала.
        /// </summary>
        protected Stack<State> States { get; set; } = new Stack<State>();

        /// <summary>
        /// Текущая точка при отрисовке фрактала.
        /// </summary>
        ...
        private void Execute(Graphics g, FractalInterpretation interpretation)
        {
            foreach (FractalCommand command in interpretation.Commands)
            {
